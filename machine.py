...
def simulate(item, store_it):
    ...
    for i in range(len(dates)):
        ...
        percent_change = percent_change*100*2
...
if __name__ == '__main__':
    ...
    if len(sys.argv)>=2 and sys.argv[1] == 'store':
        ...
        combos = []
        #(start_price, price, gamma, epsilon , c, cutoff, cutoff2) = item
        for gamma in [50.0, 100.0,150.0,200.0]:
            for epsilon in [.1,.5,1,2,3]:
                for c in [.1,.5,1.0,2.0,3.0]:
                    for cutoff in range(2,6):
                        for cutoff2 in range(3,10):
                            if cutoff2<cutoff: continue
                            combos.append([2, 25, gamma, epsilon, c, cutoff, cutoff2])
